...
p5.Plot.prototype.drawAlexHistogram = function(){
  ...
  for (let i=0; i<numBins; i++){
    binArr.push([]); // value arrays in bin arrays
  }
  ...
  let minFreq = binArr[0].length;
  let maxFreq = binArr[0].length;
  // compare the length of each value array within a bin
  for (let p=0; p<binArr.length;p++){
    let currentFreq = binArr[p].length;
  ...
  for (let q=0; q<binArr.length;q++){
    let currentFreq = binArr[q].length;
