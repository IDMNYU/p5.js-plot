  ...
  if (typeof define === 'function' && define.amd)
    ...
    tokenize(_arr) {
      ...
      for (let i=0; i<that._xkey.length; i++) {
        ...
        {
          that.plotData[j]._plotx = map(that.plotData[j][that._xkey[i]], that._xmin, that._xmax, that.padding, (that.width-that.padding)*that.aa);
          that.plotData[j]._ploty = map(that.plotData[j][that._ykey], that._ymin, that._ymax, (that.height-that.padding)*that.aa, that.padding);
