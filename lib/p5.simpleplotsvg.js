...
(function (root, factory) {
  if (typeof define === 'function' && define.amd)
    ...
    tokenize(_arr) {
      ...
      pop();
      that.svghtml += '" style="fill:none;stroke:'+sc+';stroke-width:'+(that.strokeweight*that.aa).toString()+'"/>';
    ...
    drawPointGraph(that) {
      ...
      for (let i=0; i<that._xkey.length; i++) {
        ...
        for(let j=0;j<that.plotData.length;j++)
        {
          ...
          that.svghtml += '<circle cx="' + x.toString() + '" cy="' + y.toString() + '" r="' + (that.pointdiameter*2*that.aa).toString() + '" stroke="'+sc+'" stroke-width="'+(that.strokeweight*that.aa).toString()+'" fill="'+fc+'"/>';
