...
(function (root, factory) {
  if (typeof define === 'function' && define.amd)
    ...
    get data() {
      ...
      this.svghtml = '<svg xmlns="http://www.w3.org/2000/svg" width="'+((this.right-this.left)*this.aa).toString()+'" height="'+((this.bottom-this.top)*this.aa).toString()+'">';
    ...
    draw(){
        image(this.plotImg, this.left, this.top, this.right-this.left, this.bottom-this.top);
    }
    ...
    tokenize(_arr) {
      ...
      for (let i=0; i<that._xkey.length; i++) {
        ...
        {
          that.plotData[j]._plotx = map(that.plotData[j][that._xkey[i]], that._xmin, that._xmax, 0, width*that.aa);
          that.plotData[j]._ploty = map(that.plotData[j][that._ykey], that._ymin, that._ymax, height*that.aa, 0);
      ...
      pop();
      that.svghtml += '" style="fill:none;stroke:black;stroke-width:'+(that.strokeweight*that.aa).toString()+'"/>';
    ...
    drawPointGraph(that) {
      ...
      for (let i=0; i<that._xkey.length; i++) {
        ...
        for(let j=0;j<that.plotData.length;j++)
        {
          ...
          that.svghtml += '<circle cx="' + x.toString() + '" cy="' + y.toString() + '" r="' + (that.pointdiameter*2*that.aa).toString() + '" stroke="black" stroke-width="'+(that.strokeweight*that.aa).toString()+'" fill="red"/>';
