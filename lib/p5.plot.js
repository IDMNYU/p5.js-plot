  ...
  if (typeof define === 'function' && define.amd)
	...
	p5.Plot = class {
		...
		constructor (_args) {
			...
			this.gridweight = 4;
			this._fillcolor = ['WhiteSmoke'];
			this._strokecolor = ['DarkRed'];
      ...
      //this._bins = 30; // does bin number change when faceting?
    ...
    set fillcolor (fillcolor) {
      this._fillcolor = Array.isArray(fillcolor) ? fillcolor : [fillcolor];
    };
    set strokecolor (strokecolor) {
      this._strokecolor = Array.isArray(strokecolor) ? strokecolor : [strokecolor];
    };
    set strokeweight (strokeweight) {
      this._strokeweight = Array.isArray(strokeweight) ? strokeweight : [strokeweight];
    };
    set xkey (xkey) {
    ...
    tokenize(_arr) {
      ...
      */
      ...
      console.log(that._xkey.length);

      // what happens when multiple xkeys and ykeys?
      for (let i=0; i<that._xkey.length; i++) {
        that.graphics.fill(that._fillcolor[i%that._fillcolor.length]);
        that.graphics.strokeWeight(that._strokeweight[i%that._strokeweight.length]);
        that.graphics.stroke(that._strokecolor[i%that._strokecolor.length]);
        ...
        {
          that.plotData[j]._plotx = map(that.plotData[j][that._xkey[i]], that._xmin, that._xmax, 0, that.graphics.width);
          that.plotData[j]._ploty = map(that.plotData[j][that._ykey], that._ymin, that._ymax, that.graphics.height, 0);
    ...
    drawHistogram(that){
      ...
      // in case of faceting, layering on same graph does not seem to work, so multiple mini graphs
    ...
    drawBar(that){
      console.log('bar graph, assume x is unique and y is contineous!');
    ...
    drawBar2(that){
      console.log('BAR GRAPH 2, assume x not unique and count occurences!');
      // discrete x
