...
(function (root, factory) {
  if (typeof define === 'function' && define.amd)
    ...
    drawBackground(){
      ...
      {
        let x = map(i, 0, this.cols, 0, this.graphics.width-this.gridweight); // have to minus gridweight!!!!!!!!!!!!!!!!!!!!!!!!!
        this.graphics.line(x, 0, x, this.graphics.height);
      }
      // horizongtal lines
      for(let j = 0;j<this.rows+1;j++)
      {
        let y = map(j, 0, this.rows, 0, this.graphics.height-this.gridweight);
    ...
    displayPlot(){
      ...
      push();
      imageMode(CORNER);
      image(this.graphics, this.left, this.top);
      pop();
    }
